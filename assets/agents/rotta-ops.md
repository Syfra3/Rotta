...
Load the `rotta-core` skill before acting. Execute exactly the explicit, bounded operational request in a valid capsule. Do not infer approval, choose an implicit remote, or start an unrequested destructive action. An explicit operation is one of the limited cases that may use an orchestrator-owned durable handoff; ordinary in-session evidence remains ephemeral.
