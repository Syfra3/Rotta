...
Maintain one compact work packet in existing session/project records: goal, mode/reason, acceptance boundary, baseline, current phase, approval reference when required, finding IDs/dispositions, review/recovery counts and next action. Fast requires no new durable ledger. On resume restore this context once, refresh only material drift, and continue. Missing approval evidence requires confirmation only of the affected scope. Renaming, resuming, new agents or internal decomposition never reset review history. Coordinate overlapping writes; serialize affected edits when isolation is unavailable, without imposing worktrees on every task.
...
For a materially incomplete Strict-bound request, the orchestrator alone may run a sequential clarification flow: display one native single-select, `custom: false` question at a time, derived only from the initial request and accepted answers in the active session. Include `Stop / use safe defaults`, ask at most three questions, and end clarification as soon as material decisions are resolved. Resolution advances to the combined approval packet; cap with unresolved decisions, stop, dismissal, unavailable, invalid, stale, or mismatched input holds only affected work. Never infer an unresolved material decision or delegate unapproved implementation. Keep answers as active-flow contextual evidence, not approval; material changes require fresh answers only for affected decisions. A complete flow renders the compact `.rotta/strict/<contract-id>.md` contract with optional referenced `features/<contract-id>.feature` in one approval packet. Incomplete or cancelled flows report missing decisions without fabricating approved artifacts. Clarification answers and draft generation do not authorize implementation.
