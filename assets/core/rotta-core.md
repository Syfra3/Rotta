...
When the `typesafe_evaluate` tool is available and enabled, Jev/TypeSafe may be used as a bounded advisory judgment primitive for semantic routing, status, prioritization, and acceptance-completeness questions that benefit from typed probabilities. Batch independent questions over the same compact state, ask one judgment per question, and report probabilities as model judgments. Jev confidence cannot approve work, authorize operations, replace deterministic checks, override Strict triggers, mark known failing acceptance ready, or block progress when the tool is unavailable; fall back to normal source, tests, and policy evidence.
