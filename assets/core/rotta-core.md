...
Strict mode is required for security, authentication, payments, migrations, destructive operations, public contracts, high-impact multi-component changes, or an explicit user request. Before implementation, write a compact contract under `.rotta/strict/` and obtain one explicit approval. When file extraction, source discovery, or domain reconciliation is needed to render that packet, delegate one bounded discovery capsule before the orchestrator reads implementation files or computes domain totals; consume its compact facts, strategy, affected paths, gaps, and evidence. If delegation is unavailable, use an explicit bounded source fallback rather than silently absorbing implementation work. Use approved Gherkin only when behavioral UI, public-interface, validation, authorization, destructive-confirmation, or workflow examples are material to unambiguous approval. A user may request a documented Fast-mode exception to a Strict trigger.
...
Keep the record compact. It contains: stable work ID; canonical workspace and loaded policy paths; revision; user outcome and mode/reason; compact scope/invariants and acceptance-check references; approval evidence references (user message/session and, for Strict, canonical artifact path, digest and revision); baseline/diff identity; current phase; outstanding acceptance failures and review finding IDs/dispositions; cumulative initial/delta review and recovery counts; passed checks with commands, actual results and evidence references; implemented/integrated/verified/delivered status; and next action. Reference the canonical contract or evidence file instead of duplicating its prose, commands, or raw logs. Retain closed finding dispositions and prior attempts, not just the latest success. Unknown history stays unknown, never zero by assumption.
...
For each eligible question, in the active agent turn: (1) canonicalize the workspace and any contract path; (2) create an ephemeral binding before rendering with the active session, canonical workspace, named action, allowed option set, safe outcome, and rendered identity where required; (3) render the exact current decision and its alternatives; (4) first try the host-native single-select question with exactly one item, `multiple: false`, and `custom: false`; (5) if native is unavailable or incompatible, render the bounded text fallback with the same prompt ID and options; (6) process only that native answer or exact fallback reply; and (7) compare it to the current binding. A locally reconstructable mechanical request/binding error may be corrected and retried once immediately only when contract bytes, digest, revision, session, workspace, action, and options remain unchanged. Never retry a dismissal, invalid/custom selection, stale or revised contract, substantive mismatch, or second failure. Safe diagnostics may name mismatched identity fields but never reveal bound values. Otherwise discard the answer and safe-stop. Never reuse an answer after a material revision or replacement.
...
Every terminal outcome explicitly declares one terminal state: `completed`, `blocked`, or `safely stopped`. The default user-facing report is compact: capability or blocker, acceptance evidence, review result, unresolved risk, and next meaningful action. Keep these literal fields in the work record: `Mode`; `Roles invoked`; `Human decision count`; `Tests run`; `Review result`; `Unresolved risk`; `Active elapsed time`; `Child-session count`; `Retries`; `User-waiting/external-outage time`. Show detailed counters/provenance when requested, when benchmarking, or when they materially explain a failure. State `unknown` or `unavailable` rather than inventing values.
