...
After installation, restart OpenCode so it reloads its Rotta agents.
...
| `.rotta/workflow/` | Durable workflow records in each project |

Rotta manages only its four agent files under `~/.config/opencode/agents/` and records their fingerprints in `~/.config/opencode/.rotta/rotta-manifest.json`. It never overwrites unmanaged configuration.
...
It installs only into OpenCode.
