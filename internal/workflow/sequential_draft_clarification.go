...
func (flow *StrictClarificationFlow) RecoverPendingDraft(repoRoot string, partial PendingStrictDraft, hardSpec, feature string) (PendingStrictDraft, error) {
	...
	staged = ""
	// Publishing the missing member cannot atomically retain ownership of the
	// already-validated pathname. Verify both final paths before reporting a
	// completed pair; if either changed, retain every on-disk member and return
	// a typed incomplete conflict for an explicit later decision.
	if err := validateRetainedStrictDraftArtifact(root, retainedPath, retainedContent, partial.partial.retainedID); err != nil {
		return partial, &WorkflowArtifactPairIncompleteError{Artifacts: partial.WorkflowPolicyArtifacts, Status: partial.Publication, cause: errors.Join(errWorkflowArtifactPairConflict, err), retainedID: partial.partial.retainedID}
	}
	if err := validateWorkflowArtifactContent(root, missingPath, missingContent); err != nil {
		return partial, &WorkflowArtifactPairIncompleteError{Artifacts: partial.WorkflowPolicyArtifacts, Status: partial.Publication, cause: errors.Join(errWorkflowArtifactPairConflict, err), retainedID: partial.partial.retainedID}
	}
...
func validateWorkflowArtifactContent(root *os.Root, path, expected string) error {
	file, err := root.Open(filepath.FromSlash(path))
	if err != nil {
		return fmt.Errorf("strict draft recovery published artifact mismatch: %w", err)
	}
	defer file.Close()
	content, err := io.ReadAll(file)
	if err != nil {
		return fmt.Errorf("read published strict draft artifact: %w", err)
	}
	if string(content) != expected {
		return fmt.Errorf("strict draft recovery published artifact content mismatch")
	}
	return nil
}
