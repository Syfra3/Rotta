...
func (m Model) viewWelcome() string {
	...
	b.WriteString(titleStyle.Render("Uncle Bob Workflow Installer") + "\n")
...
func (m Model) viewTargetSelect() string {
	...
	items := []struct {
		...
		{"Claude Code", "SKILL.md files → ~/.claude/skills/uncle-bob/"},
		{"OpenCode", "Agent files (bob-orchestrator, bob-spec, bob-impl, bob-review) → ~/.config/opencode/agent/"},
...
func (m Model) viewAncora() string {
	...
	b.WriteString(menuItemStyle.Render("  Bob agents call ancora_save / ancora_search to remember decisions") + "\n")
...
func (m Model) viewQualityGates() string {
	...
	options := []struct {
		...
		{"Use defaults (recommended)", "Reasonable starting thresholds, editable in .uncle-bob/quality-gates.yaml"},
...
func (m Model) viewConfirm() string {
	...
	if m.Target == "claude-code" || m.Target == "both" {
		if m.SelectedModes[0] {
			b.WriteString(menuItemStyle.Render("  ~/.claude/skills/uncle-bob/spec-mode/SKILL.md") + "\n")
		}
		if m.SelectedModes[1] {
			b.WriteString(menuItemStyle.Render("  ~/.claude/skills/uncle-bob/implementation-mode/SKILL.md") + "\n")
		}
		if m.SelectedModes[2] {
			b.WriteString(menuItemStyle.Render("  ~/.claude/skills/uncle-bob/review-mode/SKILL.md") + "\n")
		}
	}
	if m.Target == "opencode" || m.Target == "both" {
		b.WriteString(menuItemStyle.Render("  ~/.config/opencode/agent/bob-orchestrator.md") + "\n")
		if m.SelectedModes[0] {
			b.WriteString(menuItemStyle.Render("  ~/.config/opencode/agent/bob-spec.md") + "\n")
		}
		if m.SelectedModes[1] {
			b.WriteString(menuItemStyle.Render("  ~/.config/opencode/agent/bob-impl.md") + "\n")
		}
		if m.SelectedModes[2] {
			b.WriteString(menuItemStyle.Render("  ~/.config/opencode/agent/bob-review.md") + "\n")
		}
	}
	b.WriteString(menuItemStyle.Render("  .uncle-bob/state-machine.yaml") + "\n")
	b.WriteString(menuItemStyle.Render("  .uncle-bob/quality-gates.yaml") + "\n")
	if m.SetupAncora {
		if m.Target == "claude-code" || m.Target == "both" {
...
func (m Model) viewInstalling() string {
	...
	b.WriteString(fmt.Sprintf("  %s Installing Uncle Bob workflow\n\n", m.InstallSpinner.View()))
...
func (m Model) viewSuccess() string {
	var b strings.Builder
	b.WriteString(successStyle.Render("✓ Uncle Bob Workflow Installed") + "\n\n")
	...
	b.WriteString(menuItemStyle.Render("  1. In your project, run /bob-spec-mode to start a feature spec") + "\n")
	...
	b.WriteString(menuItemStyle.Render("  4. After TDD, run /bob-review-mode for quality gate evaluation") + "\n\n")
