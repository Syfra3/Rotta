...
- Phase 4 — Review: run the metrics-based review workflow. The Judge reviews evidence, not code. Resolve only the ` + "`rotta.quality-gates/v2 generic gate plan`" + ` from the recorded snapshot and persist current review evidence to ` + "`.rotta/current/review-evidence.yaml`" + `.
- An unresolved required gate command blocks review; it is never passed, not applicable, guessed, or substituted. Use only .rotta/current/tdd-log.md as active TDD evidence; root and archived logs are not review authority.
- Derive readiness only from matching persisted current review evidence. Waivers remain visible as ` + "`waived`" + ` rather than passed, and a ready or ready_with_waivers result binds ` + "`reviewed_commit`" + ` before entering ` + "`final_human_review`" + `; explicit human approval remains required for completion.
- Generate manual PR handoff only from matching persisted current review evidence. After Phase 4 passes, provide a testable manual GitHub PR handoff: print the absolute recorded worktree path and git status --short; when reviewed outstanding changes exist, print optional reviewed-path-only git add -- <paths> and commit commands; print the resolved git push <remote> feature/<slug> command and gh pr create --base <base-branch> --head feature/<slug>; offer the GitHub web UI alternative; and disclose active-host command/delegation limits and that credentials remain user-controlled.
...
- For every phase-work request, supported hosts direct the workflow to shared workspace authority and the Rotta-Orchestrator, preserving canonical approval gates and legal transitions. Retired legacy markers never authorize workflow activity. Final approval retains final_human_review semantics: only explicit human approval of the unchanged reviewed snapshot completes the feature.
...
- Artifact installation does not require a local Claude executable and makes no runtime compatibility verification claim.
